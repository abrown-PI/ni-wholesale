// NI Wholesale — Notify Order Edge Function (v21, bill External ID always matches invoice prefix)
...
async function loadSiteSettings(supa: ReturnType<typeof createClient>): Promise<Record<string, string>> { const out: Record<string, string> = {}; try { const r = await supa.from('settings').select('key,value').in('key', ['accounting_notify_email','ni_vendor_id_in_pi','default_expense_account_id','company_name']); (r.data || []).forEach((row: { key: string; value: unknown }) => { out[row.key] = typeof row.value === 'string' ? row.value : String(row.value ?? ''); }); } catch { /* fall through */ } return out; }
...
async function generateCsv(supa: ReturnType<typeof createClient>, key: 'invoice' | 'bill', order: Record<string, unknown>, customer: Record<string, unknown>, location: Record<string, unknown> | null, items: Record<string, unknown>[], settings: Record<string, string>): Promise<{ csv: string; filename: string }> { const settingsKey = key === 'invoice' ? 'netsuite_csv_mapping' : 'netsuite_bill_csv_mapping'; const fallback = key === 'invoice' ? DEFAULT_INVOICE_MAPPING : { columns: [] }; const mapping = await loadMapping(supa, settingsKey, fallback); const cols = mapping.columns || []; if (!cols.length) throw new Error(`${key} CSV mapping has no columns configured`); /* External ID MUST come from the invoice mapping for both invoice and bill so intercompany pairs stay linked when the prefix changes (e.g., NI- → NIINV). */ const invoiceMapping = key === 'invoice' ? mapping : await loadMapping(supa, 'netsuite_csv_mapping', DEFAULT_INVOICE_MAPPING); const pad = Number(invoiceMapping.external_id_pad_digits) || 5; const prefix = invoiceMapping.external_id_prefix || 'NI-'; const orderNumPadded = String(order.order_number || '').padStart(pad, '0'); const externalId = prefix + orderNumPadded; const externalCustomerId = 'NI-CUST-' + String(customer.id || '').slice(0, 8); const invoiceDate = fmtDateUS(String(order.shipped_at || order.placed_at || '')); const terms = String(customer.payment_terms || 'Net 30'); const dueDate = addDays(String(order.shipped_at || order.placed_at || ''), termsToDays(terms)); const memoBits = [`NI Wholesale order #${orderNumPadded}`]; if (order.po_number) memoBits.push(`PO#: ${order.po_number}`); if (order.tracking_number) memoBits.push(`${order.carrier || 'Tracking'}: ${order.tracking_number}`); const memo = memoBits.join(' | '); const firstItemExpAcct = key === 'bill' ? await firstItemExpenseAccount(supa, items) : ''; function renderRow(item: Record<string, unknown> | null, lineType: LineType): string[] { const ctx = { order, customer, location: location || {}, item, lineType, mapping, settings, externalId, externalCustomerId, invoiceDate, dueDate, memo, firstItemExpenseAccount: firstItemExpAcct }; return cols.map((c) => c.source === 'static' ? String(c.value || '') : resolve(c.source, ctx)); } const rows: string[][] = []; for (const it of items) rows.push(renderRow(it, 'item')); if (order.shipping && Number(order.shipping) > 0) rows.push(renderRow(null, 'shipping')); if (order.tax && Number(order.tax) > 0) rows.push(renderRow(null, 'tax')); if (key === 'invoice' && order.discount_amount && Number(order.discount_amount) > 0) rows.push(renderRow(null, 'discount')); let csv = cols.map((c) => csvEscape(c.name)).join(',') + '\r\n'; for (const r of rows) csv += r.map(csvEscape).join(',') + '\r\n'; const filename = key === 'invoice' ? `ni-invoice-${orderNumPadded}.csv` : `ni-bill-${orderNumPadded}.csv`; return { csv, filename }; }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const action = body?.action as ('placed' | 'shipped' | 'customer-csv') | undefined;
    ...
    } else return new Response(JSON.stringify({ error: 'invalid action; must be placed, shipped, or customer-csv' }), { status: 400, headers: jsonHeaders });
