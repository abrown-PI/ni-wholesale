...
function resolve(sourceKey: string, ctx: Record<string, unknown>): string {
  ...
  switch (sourceKey) {
    ...
    case 'computed.invoice_external_id': return String(ctx.invoiceExternalId || ctx.externalId || '');
...
async function generateCsv(supa: ReturnType<typeof createClient>, key: 'invoice' | 'bill', order: Record<string, unknown>, customer: Record<string, unknown>, location: Record<string, unknown> | null, items: Record<string, unknown>[], settings: Record<string, string>): Promise<{ csv: string; filename: string }> { const settingsKey = key === 'invoice' ? 'netsuite_csv_mapping' : 'netsuite_bill_csv_mapping'; const fallback = key === 'invoice' ? DEFAULT_INVOICE_MAPPING : { columns: [] }; const mapping = await loadMapping(supa, settingsKey, fallback); const cols = mapping.columns || []; if (!cols.length) throw new Error(`${key} CSV mapping has no columns configured`); /* External ID: bill prefers its OWN prefix+pad if the admin set them (e.g., NIBILL so PI's NetSuite can tell NI vendor bills apart from Reviv's). If bill's fields are blank, fall back to the invoice mapping so today's intercompany-pair convention still holds. */ const invoiceMapping = key === 'invoice' ? mapping : await loadMapping(supa, 'netsuite_csv_mapping', DEFAULT_INVOICE_MAPPING); const pad = Number(mapping.external_id_pad_digits || invoiceMapping.external_id_pad_digits) || 5; const prefix = mapping.external_id_prefix || invoiceMapping.external_id_prefix || 'NI-'; const orderNumPadded = String(order.order_number || '').padStart(pad, '0'); const externalId = prefix + orderNumPadded; const invoicePad = Number(invoiceMapping.external_id_pad_digits) || 5; const invoicePrefix = invoiceMapping.external_id_prefix || 'NI-'; const invoiceExternalId = invoicePrefix + String(order.order_number || '').padStart(invoicePad, '0'); const externalCustomerId = 'NI-CUST-' + String(customer.id || '').slice(0, 8); const invoiceDate = fmtDateUS(String(order.shipped_at || order.placed_at || '')); const terms = String(customer.payment_terms || 'Net 30'); const dueDate = addDays(String(order.shipped_at || order.placed_at || ''), termsToDays(terms)); const memoBits = [`NI Wholesale order #${orderNumPadded}`]; if (order.po_number) memoBits.push(`PO#: ${order.po_number}`); if (order.tracking_number) memoBits.push(`${order.carrier || 'Tracking'}: ${order.tracking_number}`); const memo = memoBits.join(' | '); const firstItemExpAcct = key === 'bill' ? await firstItemExpenseAccount(supa, items) : ''; function renderRow(item: Record<string, unknown> | null, lineType: LineType): string[] { const ctx = { order, customer, location: location || {}, item, lineType, mapping, settings, externalId, invoiceExternalId, externalCustomerId, invoiceDate, dueDate, memo, firstItemExpenseAccount: firstItemExpAcct }; return cols.map((c) => c.source === 'static' ? String(c.value || '') : resolve(c.source, ctx)); } const rows: string[][] = []; for (const it of items) rows.push(renderRow(it, 'item')); if (order.shipping && Number(order.shipping) > 0) rows.push(renderRow(null, 'shipping')); if (order.tax && Number(order.tax) > 0) rows.push(renderRow(null, 'tax')); if (key === 'invoice' && order.discount_amount && Number(order.discount_amount) > 0) rows.push(renderRow(null, 'discount')); let csv = cols.map((c) => csvEscape(c.name)).join(',') + '\r\n'; for (const r of rows) csv += r.map(csvEscape).join(',') + '\r\n'; const filename = key === 'invoice' ? `ni-invoice-${orderNumPadded}.csv` : `ni-bill-${orderNumPadded}.csv`; return { csv, filename }; }
